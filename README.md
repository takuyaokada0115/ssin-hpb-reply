...
| 方法 | 手順 | 特徴 |
|------|------|------|
| CSV出力（推奨） | 予約一覧で「CSV出力」→ 中身をコピーしてテキスト欄に貼り付け | メニュー名が省略されず、最も正確。画像より速く安価 |
| スクショ＋通知メール | 予約一覧のスクショ＋その日のcoming-soon予約通知メールを貼り付け | 担当者は画面から、顧客名・メニューはメールから取れる |
| 予約一覧のスクショ | 予約一覧でスタッフを絞り込み → 検索 → スクショ | 手軽。ただしメニュー名が「…」で切れる点に注意 |
| スケジュールのスクショ | スケジュール画面をスクショ | 全スタッフを一度に処理できる |
