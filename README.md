...
| 方法 | 手順 | 特徴 |
|------|------|------|
| **coming-soon 予約管理画面（推奨）** | その日のタイムテーブルをスクショ（横スクロール分は複数枚） | 担当者・顧客名・メニューが1画面に揃う。**未割り当て予約も分かる** |
| coming-soon画面＋通知メール | 上記に加えて、その日の予約通知メールをテキスト欄に貼り付け | 列幅で切れたメニュー名を全文で補完できる |
...
- 横スクロールで複数枚に分けて貼った場合、重複する時間帯の予約は1件にまとめます
